...
public:
  /**
   * The sensors are stored in a class so that they can be easily passed to the
   ...
   * // tracking wheel using a new 2.75" wheel, 0.5 inches to the right of the
   * tracking center lemlib::TrackingWheel vertical1(&vertical_rotation,
   * lemlib::Omniwheel::NEW_275, 0.5); lemlib::OdomSensors sensors(&vertical1,
   * // vertical tracking wheel nullptr, // no second vertical tracking wheel,
   * set to nullptr nullptr, // no horizontal tracking wheels, set to nullptr
   *                     nullptr, // no second horizontal tracking wheel, set to
   * nullptr &imu); // IMU
   * @endcode
   */
...
public:
   ...
   *
   * The constants are stored in a class so that they can be easily passed to
   * the chassis class Set a constant to 0 and it will be ignored
   *
   * @param kP proportional gain
   * @param kI integral gain
   * @param kD derivative gain
   * @param antiWindup integral anti windup range. If error is within this
   * range, integral is set to 0
   * @param smallError range of error at which the chassis controller will exit
   * if the error is within this range for an amount of time determined by
   * smallErrorTimeout
   * @param smallErrorTimeout the time the chassis controller will wait before
   * exiting if error is within a certain range determined by smallError
   * @param largeError range of error at which the chassis controller will exit
   * if the error is within this range for an amount of time determined by
   * largeErrorTimeout
   * @param largeErrorTimeout the time the chassis controller will wait before
   * exiting if error is within a certain range determined by largeError
   * @param slew maximum acceleration
   *
   * @b Example
   * @code {.cpp}
   * lemlib::ControllerSettings lateralSettings(10, // proportional gain (kP)
   *                                            0, // integral gain (kI), set to
   * 0 to disable 3, // derivative gain (kD), set to 3 3, // integral anti
   * windup range, set to 0 to disable 1, // small error range, in inches 100,
   * // small error range timeout, in milliseconds 3, // large error range, in
   * inches 500, // large error range timeout, in milliseconds 5); // maximum
   * acceleration (slew)
   * @endcode
   */
...
public:
   ...
   * @param trackWidth the track width of the robot, in inches. This is the
   * distance from the left wheels to the right wheels
   * @param wheelDiameter the diameter of the wheel used on the drivetrain, in
   * inches
   * @param rpm the rpm of the wheels
   * @param horizontalDrift higher values make the robot move faster but causes
   * more overshoot on turns. Recommended value of 2 if not using traction
   * wheels, 8 if using traction wheels
   *
   * @b Example
   * @code {.cpp}
   * // drive motors
   * pros::Motor lF(-5, pros::E_MOTOR_GEARSET_06); // left front motor. port 5,
   * reversed pros::Motor lM(4, pros::E_MOTOR_GEARSET_06); // left middle motor.
   * port 4 pros::Motor lB(-3, pros::E_MOTOR_GEARSET_06); // left back motor.
   * port 3, reversed pros::Motor rF(6, pros::E_MOTOR_GEARSET_06); // right
   * front motor. port 6 pros::Motor rM(-9, pros::E_MOTOR_GEARSET_06); // right
   * middle motor. port 9, reversed pros::Motor rB(7, pros::E_MOTOR_GEARSET_06);
   * // right back motor. port 7
   *
   * // motor groups
   ...
   * omnis 360, // drivetrain rpm is 360 2); // horizontalDrift is 2. If we had
   * traction wheels, it would have been 8
   * @endcode
   */
 ...
 *
 * When using swing turns, the user needs to specify what side of the drivetrain
 * should be locked we could just use an integer or boolean for this, but using
 * an enum class improves readability
 *
...
public:
  ...
  /**
   * @brief Calibrate the chassis sensors. THis should be called in the
   * initialize function
   *
   ...
   * @b Example
   * @code {.cpp}
   * // initialize function in your project. The first function that runs when
   * the program is started void initialize() { chassis.calibrate();
   * }
   * @endcode
   * @code {.cpp}
   * // initialize function in your project. The first function that runs when
   * the program is started void initialize() {
   *     // don't calibrate the IMU
   *     // this should only be necessary if you are using a different library
   * that calibrates the IMU chassis.calibrate(false);
   * }
   * @endcode
   */
   ...
   * // print the x, y, and theta values of the pose
   * printf("X: %f, Y: %f, Theta: %f\n", poseRadStandard.x, poseRadStandard.y,
   * poseRadStandard.theta);
   * @endcode
   */
   ...
   *
   * @note Units are in inches if current motion is moveToPoint, moveToPose or
   * follow, degrees for everything else
   *
   ...
   * @b Example
   * @code {.cpp}
   * chassis.setPose(0, 0, 0); // set the pose of the chassis to x = 0, y = 0,
   * theta = 0
   * // turn the robot to face the point x = 45, y = -45, with a timeout of
   * 1000ms chassis.turnToPoint(45, -45, 1000);
   * // turn the robot to face the point x = 45, y = -45, with a timeout of
   * 1000ms
   ...
   * // turn the robot to face the point x = -30, y = 22.5 with a timeout of
   * 1500ms
   * // and turn counterclockwise
   * chassis.turnToPoint(-30, 22.5, 1500, {.direction =
   ...
   * // turn the robot to face the point x = 7.5, y = 7.5 with a timeout of
   * 2000ms
   * // and a minSpeed of 60, and exit the movement if the robot is within 5
   * degrees of the target chassis.turnToPoint(7.5, 7.5, 2000, {.minSpeed = 60,
   * .earlyExitRange = 5});
   * @endcode
   */
   ...
   * // and turn counterclockwise
   * chassis.turnToHeading(-90, 1500, {.direction =
   * AngularDirection::CCW_COUNTERCLOCKWISE});
   ...
   * // and a minSpeed of 60, and exit the movement if the robot is within 5
   * degrees of the target chassis.turnToHeading(45, 2000, {.minSpeed = 60,
   * .earlyExitRange = 5});
   * @endcode
   */
  void turnToHeading(float theta, int timeout, TurnToHeadingParams params = {},
                     bool async = true);
  /**
   * @brief Turn the chassis so it is facing the target heading, but only by
   * moving one half of the drivetrain
   *
   * @param theta heading location
   ...
   * // with a minSpeed of 20 and a maxSpeed of 60
   * // and lock the right side of the drivetrain
   * chassis.swingToHeading(90, DriveSide::RIGHT, 500, {.maxSpeed = 60,
   * .minSpeed = 20});
   * // turn the robot to face heading 45 with a timeout of 2000ms
   * // and a minSpeed of 60, and exit the movement if the robot is within 5
   * degrees of the target
   * // and lock the left side of the drivetrain
   * chassis.swingToHeading(45, DriveSide::LEFT, 2000, {.minSpeed = 60,
   * .earlyExitRange = 5});
   * @endcode
   */
  ...
  /**
   * @brief Turn the chassis so it is facing the target point, but only by
   * moving one half of the drivetrain
   *
   * @param x x location
   * @param y y location
   ...
   * // with a minSpeed of 20 and a maxSpeed of 60
   * // and lock the left side of the drivetrain
   * chassis.swingToPoint(10, 10, DriveSide::LEFT, 500, {.maxSpeed = 60,
   * .minSpeed = 20});
   * // turn the robot to face the point x = 7.5, y = 7.5 with a timeout of
   * 2000ms
   * // and a minSpeed of 60, and exit the movement if the robot is within 5
   * degrees of the target
   * // and lock the right side of the drivetrain
   * chassis.swingToPoint(7.5, 7.5, DriveSide::RIGHT, 2000, {.minSpeed = 60,
   * .earlyExitRange = 5});
   * @endcode
   */
   ...
   * @b Example
   * @code {.cpp}
   * // move the robot to x = 20, y = 15, and face heading 90 with a timeout of
   * 4000ms chassis.moveToPose(20, 15, 90, 4000);
   * // move the robot to x = 20, y = 15, and face heading 90 with a timeout of
   * 4000ms
   * // but face the point with the back of the robot
   * chassis.moveToPose(20, 15, 90, 4000, {.forwards = false});
   * // move the robot to x = -20, 32.5 and face heading 90 with a timeout of
   * 4000ms
   ...
   * // move the robot to x = 7.5, y = 7.5 and face heading 90 with a timeout of
   * 4000ms
   * // with a minSpeed of 60, and exit the movement if the robot is within 5
   * inches of the target chassis.moveToPose(7.5, 7.5, 90, 4000, {.minSpeed =
   * 60, .earlyExitRange = 5});
   * // move the robot to 0, 0, and facing heading 0 with a timeout of 4000ms
   * // this motion should not be as curved as the others, so we set lead to a
   * smaller value (0.3) chassis.moveToPose(0, 0, 0, 4000, {.lead = 0.3});
   * @endcode
   */
   ...
   * // with a minSpeed of 60, and exit the movement if the robot is within 5
   * inches of the target chassis.moveToPoint(7.5, 7.5, 4000, {.minSpeed = 60,
   * .earlyExitRange = 5});
   * @endcode
   */
   ...
   * // this should also be done outside of any functions, otherwise it won't
   * compile ASSET(myPath_txt); // we replace "." with "_" to make the asset
   * name valid
   *
   * // autonomous function in your project. The function that runs during the
   * autonomous period void autonomous() {
   *     // follow the path in "myPath.txt" with a lookahead of 10 inches and a
   * timeout of 4000ms chassis.follow(myPath_txt, 10, 4000);
   *     // follow the path in "myPath.txt" with a lookahead of 10 inches and a
   * timeout of 4000ms
  ...
  /**
   * @brief Control the robot during the driver using the tank drive control
   * scheme. In this control scheme one joystick axis controls the left motors'
   * forward and backwards movement of the robot, while the other joystick axis
   * controls right motors' forward and backward movement.
   * @param left speed to move left wheels forward or backward. Takes an input
   * from -127 to 127.
   * @param right speed to move right wheels forward or backward. Takes an input
   * from -127 to 127.
   * @param disableDriveCurve whether to disable the drive curve or not. If
   * disabled, uses a linear curve with no deadzone or minimum power
   *
   * @b Example
   * @code {.cpp}
   * // opcontrol function in your project. The function that runs during the
   * driver control period void opcontrol() {
   *     // controller
   ...
   *         int leftY =
   * controller.get_analog(pros::E_CONTROLLER_ANALOG_LEFT_Y); int rightY =
   ...
   * @param disableDriveCurve whether to disable the drive curve or not. If
   * disabled, uses a linear curve with no deadzone or minimum power
   * @param desaturateBias how much to favor angular motion over lateral motion
   * or vice versa when motors are saturated. A value of 0 fully prioritizes
   * lateral motion, a value of 1 fully prioritizes angular motion
   *
   * @b Example
   * @code {.cpp}
   * // opcontrol function in your project. The function that runs during the
   ...
   *         // get joystick positions
   *         int leftY =
   * controller.get_analog(pros::E_CONTROLLER_ANALOG_LEFT_Y); int rightX =
   ...
   * @b Example (advanced)
   * @code {.cpp}
   * // this example shows how the cancelMotion function behaves when a motion
   * is queued
   ...
   * });
   * // queue a motion to x = 10, y = 10 with a timeout of 4000ms
   * // this will never run because cancelAllMotions will be called while this
   * motion is in the queue chassis.moveToPoint(10, 10, 4000);
   * @endcode
   */
  ...
  /**
   * PIDs are exposed so advanced users can implement things like gain
   * scheduling Changes are immediate and will affect a motion in progress
   *
