...
public:
   ...
   * // it's also 5 inches away from the tracking center. This tracking wheel is
   * to the left
   * // of the tracking center, so we use a negative distance. If it was to the
   * right of the
   * // tracking center, we would use a positive distance
   * lemlib::TrackingWheel verticalTrackingWheel(&verticalEncoder,
   * lemlib::Omniwheel::NEW_275, -5);
   ...
   * // it's also 2 inches away from the tracking center. This tracking wheel is
   * to the back
   * // of the tracking center, so we use a negative distance. If it was to the
   * front of the
   * // tracking center, we would use a positive distance
   * // this wheel also has a 5:3 gear ratio
   * lemlib::TrackingWheel horizontalTrackingWheel(&horizontalEncoder,
   * lemlib::Omniwheel::OLD_325, -2, 5.0/3.0);
   * @endcode
   */
  TrackingWheel(pros::adi::Encoder *encoder, float wheelDiameter,
                float distance, float gearRatio = 1);
  /**
   * @brief Create a new tracking wheel
   *
   ...
   * // note its important to include the motor gearset otherwise the gear ratio
   * will be incorrect pros::Motor lF(-5, pros::E_MOTOR_GEARSET_06); // left
   * front motor. port 5, reversed pros::Motor lM(4, pros::E_MOTOR_GEARSET_06);
   * // left middle motor. port 4 pros::Motor lB(-3, pros::E_MOTOR_GEARSET_06);
   * // left back motor. port 3, reversed
   ...
   * // distance is also negative because the left drive side is to the left of
   * the tracking center
   * // if it was to the right of the tracking center, we would use a positive
   * distance
   * // the rpm is 360
   * lemlib::TrackingWheel leftTrackingWheel(&leftMotors,
   * lemlib::Omniwheel::OLD_4, -5, 360);
   * @endcode
   */
   ...
   *
   * If you are using odometry provided by LemLib, this will automatically be
   * called when the chassis is calibrated
   *
   * @b Example
   * @code {.cpp}
   * void initialize() {
   ...
   * void initialize() {
   *     while (true) {
   *         // print the distance traveled by the tracking wheel to the
   * terminal std::cout << "distance: " <<
   * exampleTrackingWheel.getDistanceTraveled() << std::endl; pros::delay(10);
   *     }
   * }
   */
   ...
   *     lemlib::TrackingWheel exampleTrackingWheel(&exampleEncoder,
   * lemlib::Omniwheel::NEW_275, 0.5);
   *     // this prints 0.5 to the terminal, the offset of the tracking wheel
   *     std::cout << "offset: " << exampleTrackingWheel.getOffset() <<
   * std::endl;
   * }
   * @endcode
   */
   ...
   *     lemlib::TrackingWheel exampleTrackingWheel(&exampleRotationSensor,
   * lemlib::Omniwheel::NEW_275, 0.5);
   *     // this prints 0 to the terminal, because it uses a rotation sensor. If
   * it used a motor group, it would
   *     // have returned 1
