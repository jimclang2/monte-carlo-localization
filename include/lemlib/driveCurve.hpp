...
/**
 * @brief  ExpoDriveCurve class. Inherits from the DriveCurve class. This is a
 * exponential drive curve.
 *
 * see https://www.desmos.com/calculator/umicbymbnl for an interactive graph
 * see https://www.vexforum.com/t/expo-drive-lemlibs-implementation for a
...
public:
   ...
   *     lemlib::ExpoDriveCurve driveCurve(5, 12, 1.132);
   *     // input 4
   *     std::cout << driveCurve.curve(4) << std::endl; // outputs 0 because
   * input is within deadzone
   *     //  input 6
   *     std::cout << driveCurve.curve(6) << std::endl; // outputs 12, the
   * minimum output std::cout << driveCurve.curve(127) << std::endl; // outputs
   * 127, the maximum output
   * }
   * @endcode
   */
