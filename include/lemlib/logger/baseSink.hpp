 ...
 *
 * Sinks are LemLib's abstraction for destinations that logged messages can be
 * sent to. They are the backbone of the logging implementation. A sink could
 * send information to anything, to stdout, to a file, or even the UI on the
 * brain screen.
 */
class BaseSink {
public:
   ...
   * <h3> Example Usage </h3>
   * @code
   * lemlib::BaseSink combinedSink({lemlib::telemetrySink(),
   * lemlib::infoSink()}); combinedSink.info("This will be sent to both
   * sinks!");
   * @endcode
   *
   ...
   * @param level
   *
   * If messages are logged that are below the lowest level, they will be
   * ignored. The hierarchy of the levels is as follows:
   * - INFO
   * - DEBUG
   * - WARN
   * - ERROR
   ...
   * @param format The format that the message will use. Use "{}" as
   placeholders.
   * @param args The values that will be substituted into the placeholders in
   the format.
   *
   * <h3> Example Usage </h3>
   * @code
  ...
  void log(Level level, fmt::format_string<T...> format, T &&...args) {
    ...
    fmt::dynamic_format_arg_store<fmt::format_context> formattingArgs =
        getExtraFormattingArgs(message);
    ...
    std::string formattedString =
        fmt::vformat(logFormat, std::move(formattingArgs));
   ...
   */
  template <typename... T>
  void fatal(fmt::format_string<T...> format, T &&...args) {
...
protected:
   ...
   * @param format
   *
   * Changing the format of the sink changes the way each logged message looks.
   * The following named formatting specifiers can be used:
   * - {time} The time the message was sent in milliseconds since the program
   * started.
   ...
   * <h3> Example Usage </h3>
   *
   * The following code would add a {zero} formatting argument which could be
   * used in setFormat method of this sink.
   *
   * @code
   ...
   */
  virtual fmt::dynamic_format_arg_store<fmt::format_context>
  getExtraFormattingArgs(const Message &messageInfo);
