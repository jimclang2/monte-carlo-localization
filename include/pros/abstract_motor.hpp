...
enum class MotorEncoderUnits {
  degrees = 0, ///< Position is recorded as angle in degrees as a floating point
               ///< number
...
public:
   ...
   */
  virtual std::int32_t
  modify_profiled_velocity(const std::int32_t velocity) const = 0;
  ...
  /**
   * Gets a vector containing the target position(s) set for the motor(s) by the
   * user
   *
   * This function uses the following values of errno when an error state is
   * reached:
   ...
   *
   *
   * \return A vector containing the target position(s) in its encoder units or
   ...
   */
  virtual std::int32_t
  get_target_velocity(const std::uint8_t index = 0) const = 0;

  /**
   * Gets a vector containing the velocity/velocities commanded to the motor(s)
   * by the user
   *
   * This function uses the following values of errno when an error state is
   * reached:
   ...
   * ENODEV - The port cannot be configured as a motor
   *
   * \return A vector containing the motor's/motors' actual velocity/velocities
   * in RPM or PROS_ERR_F if the operation failed, setting errno.
   */
   ...
   *
   *
   * \return A vector containing 1 for moving in the positive direction, -1 for
   * moving in the negative direction, and PROS_ERR if the operation failed,
   * setting errno.
   */
   ...
   *
   *
   * \return A vector containing the motor's/motors' efficiency/efficiencies in
   * percent or PROS_ERR_F if the operation failed, setting errno.
   */
   ...
   *
   * \return A vector containing the motor's/motors' absolute position(s) in its
   encoder units or PROS_ERR_F
   * if the operation failed, setting errno.
   */
   ...
   * operation failed.
   */
  virtual std::int32_t get_raw_position(std::uint32_t *const timestamp,
   ...
   *            count was read will not be supplied
   *
   * \return A vector containing the raw encoder count(s) at the given timestamp
   * or PROS_ERR if the operation failed.
   */
  virtual std::vector<std::int32_t>
  get_raw_position_all(std::uint32_t *const timestamp) const = 0;
   ...
   */
  virtual std::int32_t
  get_current_limit(const std::uint8_t index = 0) const = 0;
   ...
   */
  virtual std::int32_t
  get_voltage_limit(const std::uint8_t index = 0) const = 0;
  ...
  virtual std::int32_t
  set_brake_mode_all(const pros::motor_brake_mode_e_t mode) const = 0;
  /**
   ...
   */
  virtual std::int32_t
  set_current_limit(const std::int32_t limit,
                    const std::uint8_t index = 0) const = 0;
  virtual std::int32_t
  set_current_limit_all(const std::int32_t limit) const = 0;
  /**
   ...
   */
  virtual std::int32_t
  set_encoder_units(const MotorUnits units,
                    const std::uint8_t index = 0) const = 0;
  virtual std::int32_t
  set_encoder_units(const pros::motor_encoder_units_e_t units,
                    const std::uint8_t index = 0) const = 0;
  virtual std::int32_t set_encoder_units_all(const MotorUnits units) const = 0;
  virtual std::int32_t
  set_encoder_units_all(const pros::motor_encoder_units_e_t units) const = 0;
  /**
  ...
  virtual std::int32_t
  set_gearing_all(const pros::motor_gearset_e_t gearset) const = 0;
   ...
   */
  virtual std::int32_t
  set_voltage_limit(const std::int32_t limit,
                    const std::uint8_t index = 0) const = 0;
  virtual std::int32_t
  set_voltage_limit_all(const std::int32_t limit) const = 0;
   ...
   */
  virtual std::int32_t
  set_zero_position(const double position,
                    const std::uint8_t index = 0) const = 0;
