...
public:
   ...
   * This function uses the following values of errno when an error state is
   * reached:
   * EADDRINUSE - The port is not configured as a digital output (e.g. the port
...
public:
   ...
   * \endcode
   */
  explicit Encoder(std::uint8_t adi_port_top, std::uint8_t adi_port_bottom,
                   bool reversed = false);
...
public:
   ...
   *gyro's returned values are correct at the beginning of autonomous/opcontrol.
   *
   * This function uses the following values of errno when an error state is
   * reached:
   * ENXIO - Either the ADI port value or the smart port value is not within its
...
public:
   ...
   * between 0-250 degrees. Potentiometer V2 rotates 330 degrees thus returning
   * an angle between 0-330 degrees.
   *
   * This function uses the following values of errno when an error state is
   * reached:
