...
public:
   ...
   *
   */
  explicit Gps(const std::uint8_t port, double xInitial, double yInitial,
  ...
  */
   ...
   * If you have 2 gps sensors, all the odd numered calls to this function will
   * return objects for the lower port number, all the even number calls will
   * return gps objects for the higher port number
   *
   *
   ...
   *
   * \return A gps object corresponding to a port that a gps sensor is connected
   * to the brain If no gps sensor is plugged in, it returns a gps sensor on
   * port PROS_ERR_BYTE
   *
   */
  static Gps get_gps();
