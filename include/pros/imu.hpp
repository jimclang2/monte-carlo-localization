...
public:
   ...
   * If you have 2 IMU sensors, all the odd numered calls to this function will
   * return objects for the lower port number, all the even number calls will
   * return IMU objects for the higher port number
   *
   *
   ...
   *
   * \return A IMU object corresponding to a port that a IMU sensor is connected
   * to the brain If no IMU sensor is plugged in, it returns a IMU sensor on
   * port PROS_ERR_BYTE
   *
   */
  static Imu get_imu();
   ...
   * \param  port
   * 				 The V5 Inertial Sensor port number from 1-21
   * \returns The physical orientation of the Inertial Sensor or PROS_ERR if an
   * error occured.
   *
   */
