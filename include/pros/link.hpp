...
public:
   ...
   * initializes.
   *
   * \note This function uses the following values of errno when an error state
   * is reached: ENXIO - The given value is not within the range of V5 ports
   * (1-21). ENODEV - The port cannot be configured as a radio. ENXIO - The
   * sensor is still calibrating, or no link is connected via the radio.
   *
   * \param port
   ...
   * \endcode
   */
  explicit Link(const std::uint8_t port, const std::string link_id,
                link_type_e_t type, bool ov = true);
   ...
   * is reached: ENXIO - The given value is not within the range of V5 ports
   * (1-21). ENODEV - The port cannot be configured as a radio. ENXIO - The
   * sensor is still calibrating, or no link is connected via the radio. EBUSY -
   * The transmitter buffer is still busy with a previous transmission, and
   * there is no room in the FIFO buffer (queue) to transmit the data. EINVAL -
   * The data given is NULL
   *
   * \param data
   ...
   * is reached: ENXIO - The given value is not within the range of V5 ports
   * (1-21). ENODEV - The port cannot be configured as a radio. ENXIO - The
   * sensor is still calibrating, or no link is connected via the radio. EINVAL
   * - The destination given is NULL, or the size given is larger than the FIFO
   * buffer or destination buffer.
   *
   * \param dest
