...
public:
   ...
   * blue_motor(3, pros::v5::MotorGears::blue); //Creates a motor on port 3 with
   * blue gear set Motor rotations_motor(4, pros::v5::MotorGears::green,
   ...
   * would result in the motor moving clockwise 10 units (counter clockwise if
   * reversed), no matter what the current position is.
   *
   * \note This function simply sets the target for the motor, it does not block
   * program execution until the movement finishes.
   *
   ...
   * parameter. This parameter can be ignored by most users but exists to give a
   * shared base class for motors and motor groups
   *
   * This function uses the following values of errno when an error state is
   * reached:
   * ENODEV - The port cannot be configured as a motor
   ...
   *
   *
   * \return A vector containing The motor's efficiency in percent or PROS_ERR_F
   * if the operation failed, setting errno.
   *
   * \b Example
   * \code
  ...
  /**
   * Gets a vector containing the absolute position of the motor in its encoder
   units.
   *
   * This function uses the following values of errno when an error state is
   * reached:
   ...
   *            count was read will not be supplied
   *
   * \return A vector containing the raw encoder count at the given timestamp or
   ...
   * ENODEV - The port cannot be configured as a motor
   *
   * \return A vector containing 1 if the temperature limit is exceeded and 0 if
   * the temperature is below the limit, or PROS_ERR if the operation failed,
   * setting errno.
   *
   * \b Example
   * \code
   ...
   * ENODEV - The port cannot be configured as a motor
   *
   * \return A vector containing the motor's current limit in mA or PROS_ERR if
   ...
   * ENODEV - The port cannot be configured as a motor
   *
   * \return A vector containing One of Motor_Units according to what is set for
   ...
   * ENODEV - The port cannot be configured as a motor
   *
   * \return A vector containing one of Motor_Gears according to what is set for
   * the motor, or pros::Motor_Gears::invalid if the operation failed.
   *
   * \b Example
   * \code
   * void initialize() {
   ...
   *
   * \return A vector containing the signed port of the motor. (negative if the
   * motor is reversed)
   */
   ...
   * motor was not reversed, or PROS_ERR if the operation failed, setting errno.
   *
   * \b Example
   * \code
   * void initialize() {
