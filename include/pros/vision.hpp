...
public:
   ...
   * -1053, -1586, 5.4, 0); vision_sensor.set_signature(EXAMPLE_SIG, &RED_SIG);
   * while (true) {
   *   vision_signature_s_t rtn = vision_sensor.get_by_sig(VISION_PORT, 0,
   * EXAMPLE_SIG);
   ...
   * void opcontrol() {
   *   pros::Vision vision_sensor(VISION_PORT);
   *   vision_color_code_t code1 = vision_sensor.create_color_code(EXAMPLE_SIG,
   * OTHER_SIG);
   * }
   * \endcode
   */
   ...
   * \code
   * void opcontrol() {
   *   std::vector<Vision> vision_all = pros::Vision::get_all_devices();  // All
   * vision sensors that are connected
   * }
   * \endcode
   */
   ...
   * 	 pros::Vision vision_sensor(VISION_PORT);
   *   vision_color_code_t code1 = vision_sensor.create_color_code(EXAMPLE_SIG,
   * OTHER_SIG); while (true) { vision_object_s_t rtn =
   * vision_sensor.get_by_code(0, code1);
   *     // Gets the largest object
   *     printf("sig: %d", rtn.signature);
   *     delay(2);
   *   }
   ...
   *   vision_object_s_t object_arr[NUM_VISION_OBJECTS];
   *   vision_color_code_t code1 = vision_sensor.create_color_code(EXAMPLE_SIG,
   * OTHER_SIG, 0, 0, 0); while (true) { vision_sensor.read_by_code(0, code1,
   * NUM_VISION_OBJECTS, object_arr); printf("sig: %d",
   * object_arr[0].signature);
   *     // Prints the signature of the largest object found
   *     delay(2);
   *   }
   * }
   ...
   * at the lowest port If this function is called multiple times, it will cycle
   * through all the ports. For example, if you have 1 vision sensor on the
   * robot this function will always return a vision sensor object for that
   * port. If you have 2 vision sensors, all the odd numered calls to this
   * function will return objects for the lower port number, all the even number
   * calls will return vision objects for the higher port number
   *
   *
   * This functions uses the following values of errno when an error state is
   * reached:
